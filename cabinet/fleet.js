...
export function checklist(f, { cabinetContracts = null } = {}) {
  ...
  steps.push({ key: 'funded', label: 'Hot key funded (zkLTC gas)', state: !purse ? 'na' : purse.balance == null ? 'warn' : Number(purse.balance) === 0 ? 'todo' : purse.low ? 'warn' : 'ok',
    detail: !purse ? 'no MatchBook configured' : purse.balance == null ? 'balance not read yet' : `${purse.balance} zkLTC ≈ ${purse.matchesLeft ?? '?'} matches at ${purse.gasPriceWei ? (Number(purse.gasPriceWei) / 1e9).toFixed(2) : '?'} gwei${purse.low ? ' — LOW: send zkLTC to the hot key (Caldera faucet)' : ''}` });
