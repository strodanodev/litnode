/** __TITLE_NAME__ — a litVM Games title. One file, five functions, a manifest.
 *
 *  Start here:  npm run create-title -- __RULESET_ID__   (made this file)
 *  Check it:    node sdk/conformance.mjs titles/__RULESET_ID__.mjs
 *  Bundle it:   node tools/bundle-title.mjs titles/__RULESET_ID__.mjs   → rulesets/__RULESET_ID__.js (+ .json with the buildHash)
 *  Host it:     RULESETS=./rulesets/__RULESET_ID__.js npm run node       → any bonded node can now fetch it by hash
 *
...
export default defineTitle({
  ...
  display: {
    title: '__TITLE_NAME__',
    url: 'https://example.com/__RULESET_ID__/',      // where the game runs; the arcade launches it here
