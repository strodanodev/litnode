...
rem A scheduled task restarts it if it dies (install-task.cmd).
cd /d "%~dp0"
...
set PORT=%RELAY_PORT%
rem Replace an earlier instance (an orphan from a closed terminal) instead of failing on the port.
...
cd /d "%AF_ROOT%"
echo [%date% %time%] relay starting on :%PORT% >> "%~dp0litnode-relay.log"
call npm run server >> "%~dp0litnode-relay.log" 2>&1
