...
rem with characters/ and .env). Logs beside this file; loops so a crash is
rem back in 5 s; one watcher per node (an earlier instance is replaced).
cd /d "%~dp0"
...
%NODE% tools\af-watch.mjs http://127.0.0.1:%PORT% >> "%LOG%" 2>&1
echo [%date% %time%] watcher exited (%errorlevel%); restarting in 5 s >> "%LOG%"
